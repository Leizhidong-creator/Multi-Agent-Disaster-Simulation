...
| `results-overview.svg` / `.png` | 量化结果图 | 展示三次固定种子的个体峰值密度、均值 ± 标准差、干预相对降幅与历史场景复现误差 | 直接读取 `docs/results/interventions.json` 和 `historical-calibration.json`；吞吐与知识库规模移至 README 表格，避免将不同证据类型堆入同一图 |
| `icons/*.svg` | 章节线性图标 | README 二级标题导航 | 同一颜色、描边与视口规范的本地 SVG，不依赖第三方图床 |
| `crowd-distribution-audit.png` | 空间演化审计图 | 以四个关键时刻展示人群向瓶颈区域汇聚的过程 | 由 `scripts/verify_phase2_visual.py` 对固定种子、96 Agent 容量、60 步事故场景生成；点表示 Pedestrian Agent，热度层表示局部占据量 |
